...
gates could not be run to a standard worth claiming on a real sample. See below.

The signing pipeline itself has been run end to end — gates, then signature,
then broadcast — using the console's simulated-sample mode, which the console
labels on screen: *"G1–G6 above were computed from modelled photons, not from
a sample. The transaction and the signature are real."* That demonstrates the
integration. It is not evidence that blood authorised a signature, and it is
